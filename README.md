...
단순히 몇 가지 문제를 푸는 퀴즈(Quiz)가 아니다. 복잡한 설치나 회원가입 없이 브라우저에서 1초 만에 실행되며, 4대 포메이션 및 16명 구장·벤치 선수 교체 자유도를 갖춘 샌드박스 위에서 **3대 경기 국면 (① 공격 전개, ② 측면 수비, ③ 경기 운영)** 별 전술을 지휘하고, 실시간 대중 여론 지지도(Vibe Meter)와 팀 밸런스(FBref 기반)의 변화를 체감할 수 있다.
