...
4. **🤖 AI 수석 코치 '차비브(Cha-Vibe)' 및 미니멀 여론 지수(Minimalist Vibe Bar)**
   * 공격수를 수비수에 배치하는 등 비정상적인 전술 기행을 벌이면 즉각 **[비상 경보]** 를 울린다.
   * 팬 지지율(`vibeScore`)은 경기 성패와 분리된 2차 스토리 지표로 운영되며, 거대 위젯 대신 하단 실시간 네티즌 채팅창 헤더에 **초슬림 3px 프로그레스 바 및 알약 배지 (`지지율 50%`)** 로 미니멀하게 통합되었다.
...
4. **UX/UI 미니멀리즘: 위계 질서 정상화 (전술 우선, 여론 축소)**
   * *이유:* 여론 지표가 화면을 과도하게 지배하는 현상을 해결하기 위해, 좌측 패널 최상단에 실제 승부의 핵심인 **팀 밸런스 진단 스탯**을 배치하고, 여론 미터기는 하단 중계창의 **슬림 바**로 압축하여 전술 시뮬레이터 본연의 직관성을 완성했다.
