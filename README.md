...
6. **🏆 1,000회 몬테카를로 매치 시뮬레이션 & 바이럴 도전 공유**
   ...
   * **2D 라이브 중계:** 확정된 후반 45분을 탑뷰 캔버스로 재생한다. 양 팀이 실제 포메이션 대형으로 움직이고, 골이 터지는 분에 중계 문구·헤더 스코어버그·골 연출이 동시에 반응한다. 동점이면 **키커 시점 승부차기 뷰**로 이어진다. 두 애니메이션 모두 이미 뽑힌 결과를 보여주는 표현 계층이라 확률에는 관여하지 않는다.
