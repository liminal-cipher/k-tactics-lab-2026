...
이 구조가 뜻하는 것은 **전술 지침만으로는 부족하고 그것을 감당할 스쿼드가 있어야 한다**는 겁니다. 남아공 시나리오의 개막 보드는 공격 70 대 남아공 수비 70이라 우위가 정확히 1.00이고, 기본 공격 루트인 하프스페이스 침투의 +0.10이 얹혀 기준선 1.10에 **정확히** 닿습니다. 개막 상태가 칼날 위에 서 있다는 뜻이라, 루트를 측면 오버랩(+0.05)으로 돌리면 1.05, 다이렉트 롱볼로 돌리면 0.99가 되어 만회골이 사라집니다. 손흥민을 벤치에 앉히거나 상대를 멕시코로 바꿔도(70/72 + 0.10 = 1.07) 마찬가지입니다. **언론이 요구한 개혁에서 벗어나는 순간 전반을 따라가지 못한다**는 것이 이 보드의 설계입니다. 기본 상태에서는 만회골이 들어가고 실점은 막지 못해, 시나리오는 하프타임 1:1로 열립니다. 하프타임 브리핑이 두 지수를 실제 수치로 보여주므로, 무엇이 모자랐는지 보고 후반에 고칠 수 있습니다.
