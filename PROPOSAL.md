...
사용자는 전술판 위에서 언론 보도를 통해 도출된 대표팀의 핵심 전술 쟁점을 3대 경기 국면으로 나누어, **4대 전술 다이얼(전개 템포·공격 루트·압박 라인·경기 성향), 감독 특수 지침 토글 2종, 하프타임 교체** 로 지휘합니다.
...
3. **국면 ③: 후반 플랜 B 및 위기관리 (스타 개인 기량 의존 탈피)**
   - **설계 의도:** 전반이 끝나야 열리는 국면입니다. 누가 방전됐는지를 체력 게이지로 확인한 뒤, **벤치에서 직접 교체**하고 다이얼을 다시 잡습니다. 교체한 선수는 후반 평균 체력을 끌어올리는 동시에 자신의 FBref 지표를 그대로 들고 들어오므로, **지친 에이스를 뺄 것인가**가 실제 상충이 됩니다(§4.4). 앱이 "누구를 언제 넣으라"고 정해 주지 않는 자리이고, **경기 중** 교체가 열리는 국면은 여기뿐입니다.
...
2. **중앙 매치데이 스테이지 (Center Stage):**
   ...
   - **AI 상대 감독 칩:** 스카우팅 브리핑과 상대 감독이 고른 카운터 전술, 그 선택이 우리 기대 득점에 곱하는 배수를 중계합니다.
   - **팀 밸런스 HUD:** 공격·수비·중원·체력 4대 속성을 피치 바로 아래 막대 4개로 상시 노출합니다 (FBref 기반 0~100). 체력 막대는 킥오프 전에는 선발 11인의 기본 체력과 다이얼이 예고하는 소모량을, 경기가 시작된 뒤에는 **지금 그라운드에 선 11인의 남은 체력**을 읽습니다. 카드의 체력 게이지, 후반 기대 득점 계산과 같은 값이라 하프타임 교체가 세 곳에서 동시에 움직입니다.
...
5. **오버레이 모달 (Modals):**
   ...
   - 시뮬레이션 모달: **2D 라이브 중계**(탑뷰 캔버스로 전반·후반 각 45분을 재생, 중계 문구와 스코어버그가 같은 시계로 갱신). 전반은 45분에 창이 스스로 닫혀 라커룸(보드)으로 돌아가고, 후반은 그대로 결과로 이어집니다. 동점이면 **키커 시점 승부차기 뷰** → 감독 명함 결과 카드. 두 애니메이션은 모두 이미 확정된 결과를 보여주는 표현 계층이라 확률을 움직이지 않습니다. 결과 카드에는 **XAI '승부 요인 TOP 3'와 접이식 'λ 분해 상세 표'** 가 포함됩니다 (§5.4).
...
| 하프타임 45' | 교체·다이얼 재조정 | 벤치에서 직접 교체. 바뀐 XI의 지표와 체력이 후반 45분 전체에 반영 |
| 후반 45~90' | 승/무/패 분포 + 최빈 스코어 | λ 다섯 항의 곱 → Poisson 1,000회 표집 |
| PK (동점 시) | 승자 | 기준선 75% ± 침착성·상대 공격력. 5명 뒤 동점이면 서든데스 |
| 결과 카드 | 그날의 실제 결과 대비 내 결과 + 감독 칭호 + 승부 요인 TOP 3 | 승·패·PK승·PK패 4분기에 여론이 한 겹 더 |
...
- **끝나도 판은 남습니다.** 명함 카드는 맨 위에 **그날의 실제 스코어와 내가 만든 스코어를 나란히** 놓습니다(남아공전이면 0-1 패배 옆에 내 결과). 상대는 코드가 아니라 국가명으로 적히고, 그 아래 한 줄이 내가 바꾼 지시를 그대로 요약합니다. **[명함 카드 저장]** 은 2배 해상도 PNG로 뽑고, **[공유하기]** 는 현재 전술을 URL로 인코딩한 **"이 전술 이겨봐" 도전 링크** 를 함께 보냅니다. **[🔄 다시 처음부터]** 는 체력과 스카우팅만 초기화하고 전술 세팅은 유지하므로, 한 군데만 고쳐 다시 돌릴 수 있습니다.
...
- **1,000회인 이유:** 표집 오차가 화면에 찍히는 정수 퍼센트 한 칸 수준(보드에 따라 ±1.1~1.3%p)으로 내려오는 지점이고, 10,000회로 늘려도 정수로 반올림하면 차이가 대부분 사라집니다. 표집기에 편향이 없다는 것은 해석적으로 계산한 승률 **10.54%** 와 20만 회 표집값 **10.52%** 를 대조해 확인했습니다(`scripts/validate_model.js`).
...
- **국면 ③은 경기 중에 옵니다.** 단계 2의 다이얼이 킥오프 전 결단이라면, 하프타임 교체는 스코어와 체력 게이지를 보고 나서 내리는 결단입니다. 같은 다이얼이라도 언제 돌리느냐가 다른 결정입니다.
