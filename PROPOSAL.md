...
2. **중앙 매치데이 스테이지 (Center Stage):**
   - **매치 컨트롤 스트립:** 포메이션(4-3-3, 3-5-2, 4-2-3-1, 4-4-2) 즉시 전환과 상대국 선택. 3개국은 실제 2026 본선 조별리그에서 만난 **체코(2-1 승)·멕시코(0-1 패)·남아공(0-1 패)** 그대로입니다. 남아공전은 스리백(3-4-3)이었으므로(FBref 경기 기록), 3-5-2로 그날의 대형을 재현하거나 포백으로 옮겨 구조 자체를 갈아엎는 선택이 모두 가능합니다.
...
이 구조가 뜻하는 것은 **전술 지침만으로는 부족하고 그것을 감당할 스쿼드가 있어야 한다**는 겁니다. 개막 보드는 공격 70 대 남아공 수비 70이라 우위가 정확히 1.00이고, 하프스페이스 침투의 +0.10을 켜야 기준선 1.10에 닿습니다. **언론이 요구한 개혁이 여전히 정공법**입니다. 반대로 손흥민을 벤치에 앉히거나 상대를 멕시코로 바꾸면(70/72 + 0.10 = 1.07) 같은 지침으로도 기준선을 넘지 못합니다. 하프타임 브리핑이 두 지수를 실제 수치로 보여주므로, 무엇이 모자랐는지 보고 후반에 고칠 수 있습니다.
