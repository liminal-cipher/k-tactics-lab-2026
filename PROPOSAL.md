...
공식이 감당하지 못한 지점도 밝혀 둡니다. **20인 80개 능력치 중 수동 보정은 김민재의 수비 한 곳뿐입니다.** 공식대로면 81이 나오는데 **90** 으로 올렸습니다. 3경기 245분 동안 인터셉트 2회, 태클 2회라는 기록은 상대가 그를 피해 반대편으로 공격했다는 뜻에 가깝습니다. 수비 지표는 관여한 횟수를 세는 방식이라 억제력이 잡히지 않고, 3경기 표본에서 그 한계가 특히 크게 드러납니다. 보정은 `scripts/parse_stats.py` 의 `override_def` 한 줄로 데이터에 남아 있어 어느 값이 손을 탄 것인지 추적할 수 있으며, 김민재의 나머지 세 능력치를 포함한 79개는 전부 공식 산출값입니다.
...
- **이 막대는 승패 그 자체가 아닙니다.** 대부분의 다이얼은 두 층에서 따로 작동합니다. 여기서는 **가산**으로 막대 길이를 바꾸고, 5.3의 기대 득점 λ에서는 **곱셈 배수**로 다시 개입합니다. 예외는 전개 템포로, 곱셈 배수 없이 이 층에서만 움직이며 바뀐 능력치를 통해 λ에 간접 반영됩니다. 유저가 읽는 지표와 승패를 정하는 확률은 스케일이 다르기 때문이며, 5.4의 요인 분해는 곱셈 층만을 대상으로 합니다.
...
이 구조가 뜻하는 것은 **전술 지침만으로는 부족하고 그것을 감당할 스쿼드가 있어야 한다**는 겁니다. 남아공 시나리오의 개막 보드는 공격 70 대 남아공 수비 70이라 우위가 정확히 1.00이고, 기본 공격 루트인 하프스페이스 침투의 +0.10이 얹혀 기준선 1.10에 **정확히** 닿습니다. 개막 상태가 칼날 위에 서 있다는 뜻이라, 루트를 측면 오버랩(+0.05)으로 돌리면 1.05, 다이렉트 롱볼로 돌리면 0.99가 되어 만회골이 사라집니다. 손흥민을 벤치에 앉히거나 상대를 멕시코로 바꿔도(70/72 + 0.10 = 1.07) 마찬가지입니다. **언론이 요구한 개혁에서 벗어나는 순간 전반을 따라가지 못한다**는 것이 이 보드의 설계입니다. 하프타임 브리핑이 두 지수를 실제 수치로 보여주므로, 무엇이 모자랐는지 보고 후반에 고칠 수 있습니다.
...
**전술 다이얼 배수 `m_d`.** 선택한 항목의 배수만 곱해지며 나머지는 1입니다. 전개 템포는 여기에 없습니다. 그 다이얼은 5.2의 가산 층에서만 작동해 능력치를 바꾸고, λ에는 그 능력치를 통해 들어갑니다.
