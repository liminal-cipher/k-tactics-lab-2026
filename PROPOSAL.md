...
**개막 보드는 기준선에 정확히 얹혀 있습니다.** 공격 70 대 남아공 수비 70이라 우위가 1.00이고, 기본 루트인 하프스페이스 침투의 +0.10이 더해져 1.10입니다. 칼날 위라 어디로든 벗어나면 만회골이 사라집니다. 루트를 측면 오버랩으로 돌리면 1.05, 손흥민을 빼거나 상대를 멕시코로 바꿔도 1.07이라 넘지 못합니다. **전술 지침만으로는 부족하고 그것을 감당할 스쿼드가 있어야 한다**는 뜻입니다. 반면 수비 우위는 기준선에 못 미쳐 실점을 막지 못하므로, 시나리오는 하프타임 1:1로 열립니다. 브리핑이 두 지수를 수치로 보여주므로 후반에 무엇을 고칠지 알 수 있습니다.
...
포메이션도 얹힙니다(4-3-3 +5, 4-2-3-1 +4, 3-5-2 +2, 4-4-2 −2). 개혁 성향의 지침이 왼쪽에, 수비적 선택이 오른쪽에 모입니다. 다만 오른쪽 두 항목은 성격이 다릅니다. **손흥민 벤치 −18은 실제 남아공전 논란에서 가져온 값이고, 비정상 포지션 배치 −25~−30은 전술이 아니라 배치 오류에 붙는 페널티**입니다.
